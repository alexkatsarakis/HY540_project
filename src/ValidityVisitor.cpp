...
#define IMPL_VISIT(type) void ValidityVisitor::Visit##type(const Object &node) {}
    IMPL_VISIT(Program)
...
void ValidityVisitor::VisitReturn(const Object &node) {
    ...
    std::cout << "DONT USE FCKING RETURN LIKE THAT MORON" << std::endl;
    exit(0);
}
...
void ValidityVisitor::VisitBreak(const Object &node) {
    ...
    std::cout << "DONT USE FCKING BREAK LIKE THAT MORON" << std::endl;
    exit(0);
}
...
void ValidityVisitor::VisitContinue(const Object &node) {
    ...
    std::cout << "DONT USE FCKING CONTINUE LIKE THAT MORON" << std::endl;
    exit(0);
}