...
idlist : ID comma_ids   { $$ = ParseIdList(ParseSimpleID($1), $2); }
       | ID ASSIGN expr { $$ = ParseIdList(ParseAssign(ParseSimpleID($1), ParseExpr($3))); }
       | /* Empty */    { $$ = ParseEmptyIdlist(); }
       ;

comma_ids : comma_ids COMMA ID              { $$ = ParseCommaIds($1, ParseSimpleID($3)); }
          | comma_ids COMMA ID ASSIGN expr  { $$ = ParseCommaIds($1, ParseAssign(ParseSimpleID($3), ParseExpr($5))); }
          | /* Empty */                     { $$ = ParseEmptyIdlist(); }
          ;
